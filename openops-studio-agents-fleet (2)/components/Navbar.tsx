...
interface NavbarProps {
  ...
  isDark: boolean;
  toggleTheme: () => void;
  onOpenWishlist: () => void; // New Prop
}

export const Navbar: React.FC<NavbarProps> = ({ cartCount, onOpenCart, onNavigate, isDark, toggleTheme, onOpenWishlist }) => {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const { user, isAuthenticated, logout } = useUser();
  const { isFavorite } = useFavorites(); // We might need a count here later, but standard hook access is fine
          ...
          <div className="flex items-center justify-between h-16 sm:h-20">
            ...
            <div className="hidden md:flex items-center gap-3">
               ...
               <button 
                  ...
                  className="p-2.5 text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-full transition-colors group"
                  title="Command Deck (Wishlist)"
               >

...
export const useShopController = () => {
  ...
  // Routing state - Removed 'blog'
  const [currentView, setCurrentView] = useState<'home' | 'about' | 'product' | 'docs' | 'stack' | 'licenses' | 'account' | 'login' | 'signup' | 'privacy' | 'terms' | 'support'>('home');
  ...
  const handleNavigate = (page: string) => {
    if (['home', 'about', 'docs', 'stack', 'licenses', 'account', 'login', 'signup', 'privacy', 'terms', 'support'].includes(page)) {

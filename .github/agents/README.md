...
**Status**: Active  
**Integration**: GitHub Copilot Chat with AI Model Selection

---

## 🎯 Overview

GitHub Copilot Spec-Kit agents provide modal-based workflows for spec-driven development in PixelForge Studio. These agents mirror the Claude slash commands (`.claude/commands/speckit.*`) but are optimized for GitHub Copilot's agent system with intelligent AI model selection based on task requirements.

### 🤖 AI Model Selection

Each agent is configured with optimal AI models based on comprehensive benchmarking:

- **See [AI-MODEL-SELECTION.md](../AI-MODEL-SELECTION.md)** for detailed model research and selection criteria
- Models are selected based on: Intelligence (40%), Speed (25%), Cost (20%), Specialization (15%)
- All configurations include fallback models for high availability
...
| Agent | File | AI Model | Intelligence | Speed | Description | Handoffs |
|-------|------|----------|--------------|-------|-------------|----------|
| **SpecKit Specify** | `speckit-specify.md` | Claude Opus 4.5 | 49 | 82 t/s | Create feature specification from natural language | → Plan, Clarify |
| **SpecKit Clarify** | `speckit-clarify.md` | Claude Opus 4.5 | 49 | 82 t/s | Resolve specification ambiguities through targeted questions | → Plan, Specify |
| **SpecKit Plan** | `speckit-plan.md` | GPT-5.2 (xhigh) | 51 | 117 t/s | Create technical implementation plan | → Tasks, Checklist |
| **SpecKit Tasks** | `speckit-tasks.md` | Claude 4.5 Sonnet | 42 | 80 t/s | Generate actionable task breakdown | → Implement, Analyze |
| **SpecKit Implement** | `speckit-implement.md` | KAT-Coder-Pro V1 | 36 | 62 t/s | Execute implementation by processing tasks | → Test, Review |
| **SpecKit Constitution** | `speckit-constitution.md` | GPT-5.2 (xhigh) | 51 | 117 t/s | Manage project governance principles | → Specify |
| **SpecKit Analyze** | `speckit.analyze.agent.md` | DeepSeek V3.2 | 41 | 31 t/s | Code review and analysis | → Implement |
| **SpecKit Checklist** | `speckit.checklist.agent.md` | Gemini 3 Flash | 46 | 220 t/s | Fast checklist validation | → Implement |

*Intelligence scores from Artificial Analysis Intelligence Index v4.0 (Jan 2026)*
